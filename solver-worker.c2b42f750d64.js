...
function createDataLoader(manifest,onProgress,options={}){
 ...
 const progress=(force=false)=>{const now=Date.now();if(force||now-lastProgress>100){lastProgress=now;onProgress({loaded,total:manifest.length,completedBytes,receivedBytes:completedBytes+[...partial.values()].reduce((a,b)=>a+b,0),totalBytes,cachedBytes});}};
 ...
 async function acquire(){if(active>=3)await new Promise(resolve=>waiting.push(resolve));else active++;}
...
let solver=null,tables=null,pending=null,running=false,initialized=false,loading=false,loadConfig=null,dataLoader=null;
...
async function loadTables(){
 ...
 try{
  if(loadConfig.manifest){
   dataLoader??=createDataLoader(loadConfig.manifest,progress=>postMessage({type:'loading',...progress}));
...
self.onmessage=({data})=>{
 if(data.type==='init'){loadConfig=data;initialized=true;postMessage({type:'ready',mode:'basic'});pump();loadTables();}
